...
#[cfg(test)]
use dbrain_reasoner::BuyPhase;
use dbrain_reasoner::{AbilityRole, BuildObject, ReasonerConfig, ReasonerCtx, ReasonerOptions};
use regex::Regex;
...
const HERO_TEMPO_MAX_CANDIDATES: usize = 10;
const HERO_TEMPO_CLEAR_PP: f64 = 4.5;
...
pub async fn build_entity_context(
    pool: &PgPool,
    query: &str,
    limit_events: i64,
) -> Result<JsonValue> {
...
async fn build_patch_review_context(
    ...
    let events =
        load_patch_overview_events(pool, query, clamp_i64(limit_events, 1, MAX_TIMELINE_EVENTS))
            .await?;
    let event_ids = events
        .iter()
        .filter_map(|event| event.get("id").and_then(JsonValue::as_i64))
...
pub async fn ask_context(
    ...
    let archetype_concept = plan
        .filters
        .get("archetype_concept")
    ...
    let hero_power_curve = if out_of_domain {
        ...
        hero_power_curve_ground_truth(pool, &intent, archetype_concept).await?
    };
    let hero_tempo_profile = if out_of_domain {
        JsonValue::Null
    } else {
        hero_tempo_profile_ground_truth(pool, archetype_concept, &hero_power_curve).await?
    };
    ...
    let ground_truth = json!({
        ...
        "hero_tempo_profile": hero_tempo_profile,
    ...
    result.insert(
        ...
        json!({
            ...
            "intent": intent,
            "archetype_concept": archetype_concept,
...
async fn ask_build_context(pool: &PgPool, query: &str, plan: &QueryPlan) -> Result<JsonValue> {
    ...
    Ok(json!({
        ...
        "prompt": prompt,
        "sources": std::iter::once(&build_context).chain(&build_context.variants).flat_map(|variant|variant.core.iter().chain(variant.situations.iter().flat_map(|block| block.items.iter()))).flat_map(|item| item.sources.iter()).collect::<Vec<_>>(),
        "retrieval_meta": {
...
fn reasoner_prompt_context(build: &BuildObject, requested_playstyle: Option<&str>) -> JsonValue {
    ...
    context["variants"] = json!(build
        .variants
        .iter()
        .map(compact_reasoner_variant)
        .collect::<Vec<_>>());
    context
}

fn compact_reasoner_variant(build: &BuildObject) -> JsonValue {
    ...
    json!({
        ...
        "name":build.name,"confidence":build.confidence,
        "family":build.family.as_ref().map(|family|json!({
            "id":family.id,"label":family.label,"player_matches":family.player_matches,
            "distinct_players":family.distinct_players,"distinct_authors":family.distinct_authors,
            "population_share":family.population_share,"post_patch_player_matches":family.post_patch_player_matches,
            "cohesion":family.cohesion,"axes":family.axes,"skill_order_support":family.skill_order_support,
            "imbues":family.imbues,"limitations":family.limitations,
        })),
...
fn explain_reasoner_build(build: &BuildObject, requested_playstyle: Option<&str>) -> String {
    ...
    sections.extend(
        std::iter::once(build)
            .chain(&build.variants)
            .map(explain_reasoner_variant),
    );
    sections.join("\n\n")
}

fn explain_reasoner_variant(build: &BuildObject) -> String {
    let mut sections = vec![format!(
        "{}: Kaufreihenfolge und Begründung; Sicherheit {:?}",
        build.name, build.confidence
    )];
    if let Some(family) = &build.family {
        sections.push(format!("Familie {}: {} Spieler-Matches, {} unabhängige Spieler, {} Autoren. Nach-Patch-Matches: {}. {}",family.id,family.player_matches,family.distinct_players,family.distinct_authors,family.post_patch_player_matches.map(|count|count.to_string()).unwrap_or_else(||"unbekannt".into()),family.limitations.join(" ")));
    }
    if !build.rationale.trim().is_empty() {
        sections.push(build.rationale.trim().to_string());
    }
    // Phase buckets may not reorder the actual planner purchase sequence.
    if !build.core.is_empty() {
        sections.push(format!(
            "Core – Kaufreihenfolge\n{}",
            explain_reasoner_items(build.core.iter())
        ));
    }
    if !build.ability_order.is_empty() {
        sections.push(format!(
            "Skillorder dieser Variante (Fähigkeits-IDs, Reihenfolge unverändert): {}",
            build
                .ability_order
                .iter()
                .map(|step| format!(
...
pub async fn search_mechanic_notes(pool: &PgPool, _query: &str, _limit: i64) -> Result<JsonValue> {
    if !table_exists(pool, "mechanic_notes").await?
        || !table_exists(pool, "vector_embeddings").await?
    {
...
pub async fn analyze_query(pool: &PgPool, query: &str) -> Result<QueryPlan> {
    ...
    let archetype_concept = (intent == "hero_archetype")
        .then(|| hero_archetype_concept(&query_terms))
        .flatten();
    Ok(QueryPlan {
        ...
        threats,
        intent,
        filters: json!({"archetype_concept": archetype_concept}),
...
fn hero_archetype_concept(terms: &[String]) -> Option<&'static str> {
    ...
    if contains_any_intent_term(terms, &["support", "supporter"]) {
        return Some("support");
    }
    if contains_any_intent_term(terms, &["tank", "frontline"]) {
        return Some("tank");
    }
    if contains_any_intent_term(terms, &["brawler", "bruiser"]) {
        return Some("brawler");
    }
    if contains_any_intent_term(terms, &["poke", "poken"]) {
        return Some("poke");
    }
    if contains_any_intent_term(terms, &["dive", "diver", "engage"]) {
        return Some("dive");
    }
    if contains_any_intent_term(terms, &["roam", "roaming", "roamer"]) {
        return Some("roam");
    }
    if contains_any_intent_term(terms, &["carry", "carries"]) {
        return Some("carry");
    }
    None
}

fn archetype_uses_power_curve(concept: Option<&str>) -> bool {
    matches!(concept, Some("tempo" | "scaling"))
}
...
async fn related_names_for_query(
    ...
    let rows = fetch_all(pool, &sql, params)
        .await?
        .into_iter()
        .map(decode_metadata)
        .collect();
...
async fn load_enrichments_by_event_id(
    ...
    for row in
        select_by_values_i64(pool, "patch_event_enrichments", "patch_event_id", event_ids).await?
    {
...
async fn hero_power_curve_ground_truth(
    pool: &PgPool,
    intent: &str,
    archetype_concept: Option<&str>,
) -> Result<JsonValue> {
    if intent != "hero_archetype" || !archetype_uses_power_curve(archetype_concept) {
...
async fn hero_tempo_profile_ground_truth(
    pool: &PgPool,
    archetype_concept: Option<&str>,
    power_curve: &JsonValue,
) -> Result<JsonValue> {
    if archetype_concept != Some("tempo")
        || power_curve.get("available").and_then(JsonValue::as_bool) != Some(true)
    {
        return Ok(JsonValue::Null);
    }

    let mut rows = power_curve
        .get("hero_curves")
        .and_then(JsonValue::as_array)
        .into_iter()
        .flatten()
        .filter(|row| row.get(10).and_then(JsonValue::as_str) == Some("early_skewed"))
        .collect::<Vec<_>>();
    ...
    let mut candidates = Vec::new();
    let mut excluded_curve_only = 0usize;
    let mut mechanics_unavailable = 0usize;
    for row in rows {
        let Some(name) = row.get(0).and_then(JsonValue::as_str) else {
            continue;
        };
        let delta_pp = row.get(9).and_then(JsonValue::as_f64).unwrap_or_default();
        let short_matches = row.get(3).and_then(JsonValue::as_i64).unwrap_or_default();
        let long_matches = row.get(7).and_then(JsonValue::as_i64).unwrap_or_default();
        let short_win_rate = row.get(4).and_then(JsonValue::as_f64);
        let mid_win_rate = row.get(6).and_then(JsonValue::as_f64);
        let long_win_rate = row.get(8).and_then(JsonValue::as_f64);

        let hero = match dbrain_reasoner::load_hero_model(&ctx, name).await {
            Ok(hero) => hero,
            Err(_) => {
                excluded_curve_only += 1;
                mechanics_unavailable += 1;
                continue;
            }
        };
        ...
        for ability in &hero.abilities {
            ...
            let pressure = has_mobility || has_control || has_damage;
            if pressure && !has_ultimate {
                non_ultimate_pressure += 1;
            }
            if pressure
                && ((ability.cooldown > 0.0 && ability.cooldown <= 20.0) || ability.charges >= 2)
            {
                repeatable_pressure += 1;
            }
        }

        let mechanics_support = tempo_mechanics_support(mobility, control, repeatable_pressure);
        let classification = tempo_candidate_classification(delta_pp, mechanics_support);
        if classification == "curve_only" {
            excluded_curve_only += 1;
            continue;
        }
        let mut families = vec!["early_power_curve"];
        if mobility > 0 {
            families.push("mobility");
        }
        if control > 0 {
            families.push("control");
        }
        if repeatable_pressure >= 2 {
            families.push("repeatable_pressure");
        }

        candidates.push(json!({
            "name": name,
            "classification": classification,
            "short_minus_long_pp": one_decimal(delta_pp),
            "short_matches": short_matches,
            "long_matches": long_matches,
            "short_win_rate_percent": short_win_rate,
            "mid_win_rate_percent": mid_win_rate,
            "long_win_rate_percent": long_win_rate,
            "mechanics_available": true,
            "mechanics_signal_kind": "derived_from_ability_descriptions_and_cooldowns",
            "mobility_abilities": mobility,
            "control_abilities": control,
            "damage_abilities": damage,
            "non_ultimate_pressure_abilities": non_ultimate_pressure,
            "repeatable_pressure_abilities": repeatable_pressure,
            "mechanics_support": mechanics_support,
            "supporting_signal_families": families,
        }));
    }

    candidates.sort_by(|left, right| {
        tempo_classification_rank(right)
            .cmp(&tempo_classification_rank(left))
            .then_with(|| {
                right
                    .get("short_minus_long_pp")
                    .and_then(JsonValue::as_f64)
                    .unwrap_or_default()
                    .total_cmp(
                        &left
    ...
    Ok(json!({
        ...
        "candidate_schema": {
            "classification": ["clear", "supported"],
            "supporting_signal_families": ["early_power_curve", "mobility", "control", "repeatable_pressure"]
        },
        "excluded_curve_only_count": excluded_curve_only,
        "mechanics_unavailable_count": mechanics_unavailable,
        "candidates": candidates,
        "answer_rule": "For a direct tempo-hero question, name clear candidates first and supported candidates only when useful. Do not promote curve_only candidates to typical tempo heroes."
    }))
}
...
fn tempo_mechanics_support(mobility: i64, control: i64, repeatable_pressure: i64) -> &'static str {
    ...
    if conversion_tools >= 2 || (conversion_tools >= 1 && repeatable_pressure >= 2) {
        "strong"
    } else if conversion_tools >= 1 || repeatable_pressure >= 2 {
        "moderate"
    } else {
        "weak"
    }
...
fn tempo_candidate_classification(delta_pp: f64, mechanics_support: &str) -> &'static str {
...
fn tempo_classification_rank(value: &JsonValue) -> u8 {
    match value.get("classification").and_then(JsonValue::as_str) {
        Some("clear") => 3,
        Some("supported") => 2,
        Some("curve_only") => 1,
        _ => 0,
...
async fn load_entity_claim_rows(
    ...
    for row in fetch_all(pool, &text_sql, values)
        .await?
        .into_iter()
...
async fn known_entity_names(pool: &PgPool) -> Result<HashSet<(String, String)>> {
    ...
    for row in fetch_all(
        pool,
        "SELECT entity_type, canonical_name FROM brain.entities",
        vec![],
    )
    .await?
    {
...
mod tests {
    ...
    fn reasoner_explanation_keeps_item_order_and_evidence_without_inventing_style() {
        ...
        let mut alternative = build.clone();
        alternative.name = "Observed alternative".into();
        alternative.core = vec![item(44, "Alternative Item", "Separate mechanical plan")];
        alternative.core[0].imbue_target = Some(202);
        alternative.ability_order = vec![dbrain_reasoner::AbilityStep {
            ability_id: 202,
            currency_type: 2,
            delta: -1,
        }];
        alternative.situations.clear();
        build.ability_order = vec![dbrain_reasoner::AbilityStep {
            ability_id: 101,
            currency_type: 2,
            delta: -1,
        }];
        build.variants.push(alternative);
        let compact = reasoner_prompt_context(&build, Some("tank"));
        assert_eq!(compact["variants"].as_array().unwrap().len(), 1);
        assert_eq!(compact["variants"][0]["name"], "Observed alternative");
        assert_eq!(compact["variants"][0]["core"][0]["item_id"], 44);
        assert_eq!(compact["variants"][0]["core"][0]["imbue_target"], 202);
        assert_eq!(
            compact["variants"][0]["ability_order"][0]["ability_id"],
            202
        );
        assert_eq!(compact["ability_order"][0]["ability_id"], 101);
        assert!(!compact["core"]
            .as_array()
            .unwrap()
            .iter()
            .any(|item| item["item_id"] == 44));
        let text = explain_reasoner_build(&build, Some("tank"));
        assert!(text.contains("Observed alternative"));
        assert!(text.contains("Alternative Item: Separate mechanical plan"));
        assert!(text.contains("202"));
        assert!(!text.contains("noch nicht berechnet"));
        assert!(text.find("First Item").unwrap() < text.find("Alternative Item").unwrap());
    }
    ...
    async fn load_known_item_names_matches_catalog() {
        ...
        let raw_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM brain.item_catalog")
            ...
            .await
            .expect("count");
        let distinct_names: i64 = sqlx::query_scalar(
            "SELECT COUNT(DISTINCT name) FROM brain.item_catalog WHERE trim(name) <> ''",
        )
        .fetch_one(&pool)
        .await
    ...
    async fn timeline_patch_events_keep_best_match_entity_type() {
        ...
        assert!(
            hollow_rows > 0,
            "fixture must contain Hollow Point item rows"
        );
    ...
    async fn save_review_roundtrip_inserts_and_cleans_up() {
        ...
        assert!(
            ...
            "expected context_hash in saved note"
        );
    ...
    #[test]
    fn hero_archetype_concept_routes_only_temporal_questions_to_power_curve() {
        for (query, expected) in [
            ("Welche Heros sind Tempo Charaktere?", Some("tempo")),
            ("Welche Tempo-Charaktere sind gut?", Some("tempo")),
            ("Welche Helden snowballen früh?", Some("tempo")),
            ("Which heroes are scaling carries?", Some("scaling")),
        ...
        assert!(!archetype_uses_power_curve(Some("support")));
        assert!(!archetype_uses_power_curve(Some("tank")));
        assert!(!archetype_uses_power_curve(None));
    }

    #[test]
    fn tempo_needs_independent_mechanics_support_instead_of_short_game_winrate_alone() {
        assert_eq!(tempo_mechanics_support(1, 1, 1), "strong");
        assert_eq!(tempo_mechanics_support(1, 0, 0), "moderate");
    ...
    fn hero_group_questions_get_semantic_archetype_intent() {
        for query in [
            ...
            "Welche Tempo-Charaktere sind typisch?",
    ...
    fn patch_overview_routing_accepts_patch_related_meta_question_from_classifier() {
        for (query, expected) in [
            ...
            assert_eq!(
                should_use_patch_overview_context(&plan, &AskEntityClaimMatch::default(), &intent),
                expected,
                "{query}"
            );
        }
    ...
    fn patch_overview_aggregates_meta_signals() {
        ...
        assert_eq!(
            overview["item_and_ability_changes"]
                .as_array()
                .map(Vec::len),
            Some(1)
        );
        assert_eq!(
            overview["objective_and_economy_changes"]
                .as_array()
                .map(Vec::len),
            Some(1)
        );
