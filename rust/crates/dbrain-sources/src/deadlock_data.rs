...
async fn pull_deadlock_data_inner(
    ...
    for rel in repo.raw_hashes.keys().filter(|rel| rel.ends_with(".json")) {
        let mut ir = crate::external::SourceIr::from_json(
            SOURCE, &format!("{REPO_WEB_URL}/blob/{}/{rel}", repo.commit_sha), PARSER_REVISION,
            crate::external::SourceRevision::Git { commit: repo.commit_sha.clone() },
            deadlock_brain_core::now_epoch_seconds()?, fs::read(snapshot.path().join(rel))?,
        )?;
        ir.add_origin(&format!("{REPO_WEB_URL}@{}:{rel}", repo.commit_sha))?;
        if let Ok(payload) = ir.payload() {
            let entity_file = ["hero-data", "ability-data", "ability-cards", "item-data", "item-cards", "npc-data"]
                .iter().any(|name| rel == &format!("data/json/{name}.json"));
            let invalid_entities = entity_file && object_entries(payload).is_err();
            if !payload.is_object() && !payload.is_array() { ir.quarantine("unknown_git_json_container"); }
            if invalid_entities { ir.quarantine("invalid_git_entity_structure_or_identity"); }
        }
        if ir.is_quarantined() {
            store.persist_ir(rel, "deadlock-data quarantined blob", &ir, &json!({"role":"game_state"})).await?;
        }
    }
    ...
    let ability_cards_value = read_json_optional(snapshot.path(), "data/json/ability-cards.json")?
...
fn read_repo_info(snapshot: &Path, pinned: &crate::git_source::PinnedRepository) -> Result<RepoInfo> {
    ...
    Ok(RepoInfo {
        repo_dir: snapshot.to_path_buf(), commit_sha: pinned.commit().into(),
        commit_time: Some(info.commit_time.to_string()), version, version_text,
        git_summary: json!({"updated":false,"mode":"pinned_objects","revision":info}), raw_hashes,
...
fn object_entries(value: &Value) -> Result<Vec<(String, Value)>> {
    let entries: Vec<(String, Value)> = match value {
        Value::Object(object) => object.iter().map(|(key,value)|(key.clone(),value.clone())).collect(),
        Value::Array(array) => array.iter().map(|value| {
            let key=value.get("Key").and_then(Value::as_str).filter(|s|!s.trim().is_empty())
                .ok_or_else(||SourcesError::invalid_input("Git entity array needs an explicit Key; no positional IDs"))?;
            Ok((key.to_owned(),value.clone()))
        }).collect::<Result<Vec<_>>>()?,
        _ => return Err(SourcesError::invalid_input("Git entity container must be map or keyed array")),
    };
    let mut seen=std::collections::BTreeSet::new();
    for (key,value) in &entries {
        if key.trim().is_empty() || !value.is_object() || !seen.insert(key.clone()) {
            return Err(SourcesError::invalid_input("invalid/duplicate Git entity identity"));
        }
    }
...
fn metadata_for(repo: &RepoInfo, rel: &str) -> Value {
    let provenance = crate::external::Provenance {
        source: SOURCE.into(), locator: format!("{REPO_WEB_URL}/blob/{}/{rel}", repo.commit_sha),
        source_revision: crate::external::SourceRevision::Git { commit: repo.commit_sha.clone() },
        parser_revision: PARSER_REVISION.into(), parser_family: "dbrain-sources".into(),
        raw_sha256: repo.raw_hashes.get(rel).cloned().unwrap_or_default(),
        schema_sha256: None, observed_at: repo.observed_at,
        origin_artifacts: std::collections::BTreeSet::from([format!("{REPO_WEB_URL}@{}:{rel}", repo.commit_sha)]),
        derivation_family: Some("deadlock-wiki/deadbot".into()),
        publication_authorized: false, provider_egress_authorized: false,
    };
    json!({
...
mod tests {
    ...
    fn entity_ids_never_fall_back_to_position() {
        ...
        assert_eq!(super::object_entries(&serde_json::json!([{"Key":"x"}])).unwrap()[0].0,"x");
    }
