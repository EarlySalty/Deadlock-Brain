...
pub fn parse_patchnotes(conn: &Connection, rebuild: bool) -> Result<Value> {
    ...
    for row in rows {
        ...
        let (events, skipped) =
            parse_patchnote_snapshot(row.id, &row.external_id, &payload, &index)?;
...
fn parse_patchnote_snapshot(
    ...
    for (line_index, raw_line) in iter_patch_lines(&content) {
        ...
        let (entity_type, entity_name, confidence, event_subject) =
            ...
            } else if let Some((entity_type, entity_name, confidence)) =
                infer_entities_from_line(&bullet_body, index)
                    .into_iter()
                    .next()
            {
                (
        ...
        let source_language = if optional_string(payload, "raw_content").is_some() {
            "en"
        } else {
            "de"
        };
        ...
        metadata.insert(
            "line_subject".to_string(),
            event_subject.clone().map_or(Value::Null, Value::String),
        );
        ...
        for name in names {
            ...
            if entity_type == "hero" || entity_type == "hero_stats_sheet" {
                index
                    .hero_names
                    .entry(key)
                    .or_insert_with(|| clean_subject(Some(&name)));
            } else {
                index
                    .item_names
                    .entry(key)
...
fn classify_change_type(text: &str) -> String {
        ...
        || lower.contains("moved from")
    {
        ...
        if mentions_positive_stat(&lower) {
            "nerf"
        } else {
            "buff"
        }
