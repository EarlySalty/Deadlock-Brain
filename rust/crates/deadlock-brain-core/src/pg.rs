...
/// Env-Variable mit dem zentralen Postgres-DSN.
pub const DSN_ENV: &str = "DEADLOCK_CENTRAL_DSN";

/// Postgres-Schema mit den Brain-Tabellen.
pub const SCHEMA: &str = "brain";

/// Ein lesender Pool nutzt zuerst den bereits autorisiert geladenen DSN.
/// Fehlt er, bleibt der native Infisical-Credential-Weg verfügbar.
...
fn connect_options(dsn: &str, read_only: bool) -> Result<PgConnectOptions> {
    let mut options = PgConnectOptions::from_str(dsn)
        .map_err(|_| anyhow!("Der Datenbankzugang ist ungültig."))?;
    if read_only {
        // Als letzte Startoption setzen, damit ein gegenteiliger DSN-Parameter
        // den ausdrücklich lesenden Aufruf nicht schreibend öffnen kann.
...
mod tests {
    ...
    fn malformed_dsn_is_never_reflected_in_the_error() {
        ...
        assert_eq!(error, "Der Datenbankzugang ist ungültig.");
    }
}
