...
"$CARGO_TARGET_DIR/release/dbrain-s12-wiki-probe" analyze fixtures/pilot.capture.json >/dev/null
"$CARGO_TARGET_DIR/release/dbrain-s12-wiki-probe" delta fixtures/pilot.capture.json fixtures/pilot.capture.json >/dev/null
status=0
"$CARGO_TARGET_DIR/release/dbrain-s12-wiki-probe" analyze fixtures/pilot.capture.json --require-production-ready >/dev/null || status=$?
if [[ "$status" -ne 3 ]]; then
