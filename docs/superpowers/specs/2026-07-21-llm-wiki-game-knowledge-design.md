...
2. Wiki: `game-wiki/` als Markdown-Artefakt mit vollstaendigen Entity- und Source-Seiten.
3. Schema: `game-wiki/AGENTS.md` beschreibt Konventionen, Provenienz, Query- und Lint-Regeln.

Die Wiki-Seiten duerfen lesbar zusammenfassen, muessen aber die vollstaendige strukturierte Quelle als JSON-Codeblock behalten. Die Vollstaendigkeit liegt im Wiki auf Disk; der Prompt bekommt nur die relevantesten Seiten.

## Seitenstruktur

- `game-wiki/AGENTS.md`: Schema und Maintainer-Regeln.
- `game-wiki/index.md`: Katalog aller Seiten nach Kategorie.
- `game-wiki/log.md`: Append-only Laufprotokoll.
- `game-wiki/pages/<entity_type>/<slug>.md`: eine Seite pro aktuellem Snapshot.
...
`deadlock-brain ask-context` durchsucht `game-wiki/` lokal und fuegt relevante vollstaendige Wiki-Seiten in `ground_truth.game_knowledge` ein. Der bestehende strukturierte Kontext bleibt erhalten; Wiki-Wissen ist eine zusaetzliche Ground-Truth-Schicht.
...
- Rebuild schreibt Schema, Index, Log und Seiten.
- Rebuild nutzt aktuelle Deadlock-Data-Snapshots und keine Live-Wiki-API.
- Suche liefert vollstaendige passende Seiten, nicht nur kompakte Excerpts.
