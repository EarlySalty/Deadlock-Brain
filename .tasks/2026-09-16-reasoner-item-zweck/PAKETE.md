...
Bestehende types/hero/item/mechanics/combat/ability_interactions/item_interactions verwenden, keine zweite Engine. Für den Coverage-Einstieg existiert bereits rust/crates/dbrain-reasoner/examples/ability_coverage.rs; dessen Rohfeld-Kandidaten sind ausdrücklich noch kein Beweis simulierter Mechanik. Wiederverwenden und um tatsächliche Modell-/Sim-Pfade erweitern. Zuerst aktuellen KV-Korpus inventarisieren: Effekte/Magnitude/Einheit/Quelle, unterstützte und nicht quantifizierte Pfade, positive/negative Werte. Bedingung aus nachweisbarer Mechanik statt Itemname oder Zwecktext. Strukturiertes Vokabular mit Trigger-Art, Kanal, Hero-/PvE-Ziel, Distinct-Target-Key, Dauer/Refresh, Tickintervall, Stack-Cap/Decay, ICD, Schaden-/HP-Schwelle und aktivem Cooldown. Proc-disabled und rekursive Proc-Schleifen korrekt behandeln.
...
Gleiche unveränderte Inputhashes für Vorher/Nachher, Referenzkategorie/-version und Metrikdefinitionen exakt notieren. Achtung: das bisherige build_evaluation::Frozen enthält KEINE PopulationPrior; evaluate/holdout lesen Population live nach. Asset- UND Populationsstand müssen als fixiertes Eingabepaar belegt werden. Beide Codefassungen auf genau diesem Paar neu messen; live_baseline des alten Exports nicht als Ersatz verwenden. Ein separat exportiertes Populations-Sidecar ist kein atomarer gleicher DB-Snapshot mit dem alten Frozen-Export; Exportzeitpunkte offen angeben. Verlustfreier Prior-Roundtrip und erfolgreicher Replay ohne erreichbare DB testen, fehlende/ungültige Eingaben nicht still durch Livewerte ersetzen. Autor-Übereinstimmung und Population getrennt, keine Summenmetrik die schlechte Holdouts verbirgt. Kendall nur mit genügend vergleichbaren Rängen, ties/fehlende Werte gemäß vorhandener Definition; null nicht zu 0 umdeuten. Jaccard@12 auf derselben Definition von Kaufpfad/Finalinventar/Upgrades, nie zwischen Messungen wechseln. Auswahl- und Kaufreihenfolge getrennt ausweisen.
